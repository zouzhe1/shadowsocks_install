...
v2ray_client_config="/etc/v2ray/lbblog_v2ray_config.json"
backup="/etc/v2ray/lbblog_v2ray_backup.conf"
...
_load() {
	local _dir="/etc/v2ray/lbboy/v2ray/src/"
	. "${_dir}$@"
}
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /lbblog , 那么只需要输入 lbblog 即可"
		read -p "$(echo -e "(默认: [${cyan}lbblog$none]):")" path
		[[ -z $path ]] && path="lbblog"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}lbblog.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="lbblog.com"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 lbv2.com 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/lbboy/v2ray
		cp -rf $(pwd)/* /etc/v2ray/lbboy/v2ray
	else
		pushd /tmp
		git clone https://github.com/233boy/v2ray -b "$_gitbranch" /etc/v2ray/lbboy/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/lbboy/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/lbboy/v2ray/config/backup.conf $backup
	cp -f /etc/v2ray/lbboy/v2ray/v2ray.sh $_v2ray_sh
	chmod +x $_v2ray_sh
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=lbblog.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=lbblog.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=lbblog/=$path/" $backup
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/lbboy/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/lbblog_v2ray_backup.txt && -d /etc/v2ray/lbboy/v2ray ]]; then
		echo
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/lbboy/v2ray ]]; then
		...
		备注...仅支持卸载使用我 (lbv2.com) 提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray 一键安装脚本 & 管理脚本 by lbv2.com .........."
	echo
	echo "帮助说明: https://lbv2.com/post/1/"
	echo
	echo "搭建教程: https://lbv2.com/post/2/"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
